...
public class FutureEx2 {
    ...
    private static final Logger logger = LoggerFactory.getLogger(FutureEx2.class.getName());
...
//        });
        ...
        Future<String> async = executorService.submit(() -> {
            Thread.sleep(2000);
            logger.debug("Async");
            return "hello";
        });

        logger.debug("is done {}", async.isDone());
        Thread.sleep(2100);
        logger.debug("병렬처리하고 싶을때");
        logger.debug("is done {}", async.isDone());
